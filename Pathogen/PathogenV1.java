...
public class Pathogen
{
	...
	public static boolean findPaths(char [][] maze)
	{
		...
		// Let's goooooooooo!!
		if (findPaths(maze, visited, path, pathSet, startRow, startCol, height, width))
        {
            // System.out.println(pathSet.get(0));
            return true;
        }
	...
	private static boolean findPaths(char [][] maze, char [][] visited, Stack<Character> path,
                                             HashSet<String> pathSet, int currentRow, int currentCol,
	                                         int height, int width)
	{
		...
		if (visited[currentRow][currentCol] == 'e')
		{
            ...
            if (!pathSet.add(pathToString(path)))
                return false;
            else
		...
		for (int i = 0; i < moves.length; i++)
		{
			...
			if (findPaths(maze, visited, path, pathSet, newRow, newCol, height, width))
				return true;
	...
	{
		if (maze[row][col] == WALL || maze[row][col] == COVID || visited[row][col] == BREADCRUMB)
			return false;
